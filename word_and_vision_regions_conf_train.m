function [net, stats] = word_and_vision_regions_conf_train_wkshp(net, imdb, getBatch, varargin)
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    [im, labels, annotator_choice_frac, region_feats] = state.getBatch(state.imdb, batch) ;
    if opts.prefetch
    ...
    res = vl_simplenn(net, im, dzdy, res, ...
                      ...
                      'cudnn', opts.cudnn) ;
    % accumulate errors
    error= sum([error,gather(res(end).x)]);
  ...
  fprintf(' obj:%.3g', error/n) ;
  fprintf(' batch obj:%.3g', res(end).x/batchSize) ;
  fprintf('\n') ;
